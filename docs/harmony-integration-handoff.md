...
remain in [harmony-plan.md](harmony-plan.md).
...
uv run --with-requirements requirements.txt \
  python -m pytest tests/test_harmony_fusion_contract.py tests/test_acceptance.py -q
uv run --with-requirements requirements.txt \
...
| Fusion | Four 64D projected tokens, masks, joint losses, removal, gradients, and restore are fixture-tested | Real `BranchBundle` data loader and rhythm branch remain project-level work |

## External inputs still required

| Provider | Required artifact | Expected location or hand-off |
|---|---|---|
| Data-pipeline owner | Canonical official split-0 manifest with normalized IDs, split, waveform availability/path, and log-Mel path | `dataset/song_manifest.csv` plus the declared dataset root |
...
| Fusion owner/team | Ratification of shared contract v0.2 | [ADR 0001](adr/0001-concept-fusion-architecture.md) |
...
python scripts/export_harmony_regions.py dataset/song_manifest.csv \
...
```bash
python scripts/prepare_chord_benchmark_source.py <mapping.csv> \
