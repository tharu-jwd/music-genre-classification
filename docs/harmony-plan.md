...
![Pseudo-supervised harmony flow](diagrams/harmony-pseudo-supervision.svg)
...
| Waveform mapping | Code present | Notebook 06 writes availability only; a clean hosted report and decode checks are still required |
| CQT chroma candidate | Code present | `scripts/harmony_chroma.py`; shared deterministic synthetic CPU gate passes |
...
```bash
uv run --with-requirements requirements-harmony.txt \
  python -m unittest tests/test_harmony_chroma.py -v
```
...
```bash
python scripts/export_harmony_regions.py \
...
```bash
uv run --with-requirements requirements-harmony.txt \
  python scripts/benchmark_harmony_extractors.py \
...
```bash
python scripts/decide_harmony_extractor.py \
...
Produce an availability report instead of target values when audio is missing.
Notebook 06 writes `waveform_available` and the unique resolved `audio_path` back to
the canonical manifest while leaving `harmony_available=false`; waveform existence
is not the same as an accepted harmony target. After freezing a small cohort, use
`scripts/export_harmony_regions.py` to publish the exact ordered log-Mel frame and
...
```bash
uv run --with-requirements requirements-harmony.txt \
  python scripts/prepare_chord_benchmark_source.py \
...
```bash
uv run --with-requirements requirements-harmony.txt \
  python scripts/decide_chord_teacher.py \
...
```bash
CUDA_VISIBLE_DEVICES="" uv run --with-requirements requirements-harmony.txt \
  python scripts/generate_essentia_chord_estimates.py \
...
```bash
uv run --with-requirements requirements-harmony.txt \
  python scripts/evaluate_chord_teacher.py \
...
```bash
uv run --with-requirements requirements-harmony.txt \
  python scripts/materialize_harmony_target_pilot.py \
...
`scripts/harmony_alignment.py` pools only valid chroma frames into explicit token
...
`scripts/temporal_harmony_branch.py` implements the first **reference baseline** for
...
uv run --with-requirements requirements.txt \
  python scripts/cache_harmony_encoder_pilot.py \
  dataset/song_manifest.csv \
  results/cohorts/harmony_extractor_seed42.json \
...
```bash
python scripts/build_harmony_screen_dataset.py \
...
```bash
python scripts/decide_harmony_branch_screen.py --print-policy-template \
...
```bash
uv run --with-requirements requirements.txt \
  python scripts/screen_temporal_harmony_branch.py \
...
```bash
python scripts/decide_harmony_branch_screen.py \
